...
class BRITSimputation:
    def __init__(self, path=None, dataframe=None, use_gpu = torch.cuda.is_available()):
        # self.path = path
    ...
    def imputation(self, epoch=200):
        ...
        self.data_iter = get_loader('./module/data/traffic.json', batch_size=64)
    ...
    def plot(self):
        ...
        plt.plot(self.df["value"], label="real", zorder=10)
...
def makedata(df):
    ...
    rec = json.dumps(rec)
    fs = open('./module/data/traffic.json', 'w')
    fs.write(rec)    
    fs.close()
    return length
